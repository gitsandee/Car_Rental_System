...
MongoClient.connect(
  "mongodb://127.0.0.1:27017/vehicleRent",
  (err, database) => {
    ...
    app.post("/formSubmissions/update", (req, res) => {
      console.log(req.body.query, req.body.newVals)
      db.collection("formSubmissions").updateOne(req.body.query, { $set: { formDetails: req.body.newVals } }, (err, result) => {
        if (err) return res.send(400, { message: err });
        res.send(200, { message: "ok" });
      });
    });
    ...
    app.get("/formSubmissions", (req, res) => {
      console.log({ req: req.body, query: req.query })
      db.collection("formSubmissions")
        .find(req.query ? { _id: req.query.id } : {})
        .toArray(function (err, result) {
          if (err) throw err;
          res.send(200, { message: "ok", data: result });
        });
